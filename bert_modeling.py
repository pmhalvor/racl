    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      with tf.variable_scope("pooler"):
        ...
        self.pooled_output = tf.layers.dense(
            ...
            config.hidden_size,
            activation=tf.tanh,
            kernel_initializer=create_initializer(config.initializer_range),trainable=iftrain)
...
def layer_norm(input_tensor, name=None):
  ...
  return tf.contrib.layers.layer_norm(
      inputs=input_tensor, begin_norm_axis=-1, begin_params_axis=-1, scope=name,trainable=iftrain)
...
def embedding_lookup(input_ids,
  ...
  embedding_table = tf.get_variable(
      ...
      initializer=create_initializer(initializer_range),trainable=iftrain)
...
def attention_layer(from_tensor,
  ...
  query_layer = tf.layers.dense(
      ...
      name="query",
      kernel_initializer=create_initializer(initializer_range),trainable=iftrain)
